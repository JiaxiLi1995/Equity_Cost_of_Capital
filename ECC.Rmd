...
# Estimate the Factor Risk Premium for FF5
## Create the second-path portfolio for estimation lambda
Second_Path <- Port_STL_s7_robust %>%
  filter(component == "Trend") %>%
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
...
## Create the second-path portfolio for estimation lambda
Second_Path <- Port_STL_s15_norobust %>%
  filter(component == "Trend") %>%
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
...
## Create the second-path portfolio for estimation lambda
Second_Path <- Port_STL_s15_robust %>%
  filter(component == "Trend") %>%
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
...
(All_FF5_ECC[,3:9] - All_FF5_ECC[,10])^2 %>%
  colSums() %>%
  as.matrix() %>%
  t() %>%
  as_tibble() %>%
  mutate(Model = "FF5") %>%
  rbind((All_PRS_ECC[,3:9] - All_PRS_ECC[,10])^2 %>%
          colSums() %>%
          as.matrix() %>%
          t() %>%
          as_tibble() %>%
          mutate(Model = "PRS")) %>%
  subset(select = c(8,1:7)) %>%

...
FM_FF5 %>%
  ...
  cbind(FM_FF5 %>%
          ...
          mutate(Factors = factor(Factors, levels = unique(.$Factors))) %>%
          group_by(Factors) %>%
          summarise(stat = toString(round(t.test(Value ~ method, data = .)$statistic, digits = 3)),
                    p_value = paste("(", format(round(t.test(Value ~ method, data = .)$p.value, digits = 2), nsmall = 2), ")", sep = "")) %>%
  ...
  cbind(FM_FF5 %>%
          ...
          mutate(Factors = factor(Factors, levels = unique(.$Factors))) %>%
          group_by(Factors) %>%
          summarise(stat = toString(round(var.test(Value ~ method, data = .)$statistic, digits = 3)),
                    p_value = paste("(", format(round(var.test(Value ~ method, data = .)$p.value, digits = 2), nsmall = 2), ")", sep = "")) %>%

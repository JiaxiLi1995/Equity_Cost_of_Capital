...
# Create the estimated portfolio beta matrix
Other_Port_FF5_Betas <- Other_Port_FF5 %>%
  rename(Alpha = RP)
...
# Assign them to Betas
for (j in 4:(length(Other_Port_FF5_Betas)-1)) {
  Other_Port_FF5_Betas[,j] <- temp_betas[,j-3]
}

# Filter the needed ones and remove Selection
Other_Port_FF5_Betas <- Other_Port_FF5_Betas %>%
  filter(Selection == TRUE) %>%
  select(-Selection)
...
Other_FF5_data <- Other_Port_STL_s7_norobust %>%
  ...
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas) %>%
  select(-Alpha) %>%
  select(-Type) %>%
  ungroup()

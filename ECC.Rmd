...
INV_sig_Intra <- Port_FF5_Betas %>%
  ...
  group_by(date) %>%
  summarise_all(list(~1/sqrt(sd(.))))

sig_Intra <- Port_FF5_Betas %>%
  select(!c(Alpha,Industry)) %>%
  group_by(date) %>%
  summarise_all(list(~sqrt(sd(.))))
...
All_FF5_ECC %>%
  subset(select = c(date, Industry, RAM, FM, FM_STL_7_N)) %>%
  filter(Industry == "Cnstr") %>%
  pivot_longer(cols = c(RAM, FM, FM_STL_7_N), names_to = "method", values_to = "ECC") %>%
  mutate(date = transDate(date)) %>%
  # fix order
  mutate(method = factor(method, levels = c("FM","FM_STL_7_N","RAM"))) %>%
  ggplot() +
  geom_line(aes(date, ECC, color = method),alpha = 0.4) +
  scale_x_date(minor_breaks = "1 year") +
  scale_color_manual(values=c("springgreen1", "blue", "red")) +

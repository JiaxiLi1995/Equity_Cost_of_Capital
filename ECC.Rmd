...
Second_Path <- Port_FF5_STL_s7_robust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(FF5_Betas_STL_s7_robust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_FF5_STL_s15_norobust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(FF5_Betas_STL_s15_norobust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_FF5_STL_s15_robust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(FF5_Betas_STL_s15_robust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_FF5_STL_s40_norobust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(FF5_Betas_STL_s40_norobust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_FF5_STL_s40_robust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(FF5_Betas_STL_s40_robust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_PRS_STL_s7_robust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(PRS_Betas_STL_s7_robust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_PRS_STL_s15_norobust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(PRS_Betas_STL_s15_norobust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_PRS_STL_s15_robust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(PRS_Betas_STL_s15_robust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_PRS_STL_s40_norobust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(PRS_Betas_STL_s40_norobust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
Second_Path <- Port_PRS_STL_s40_robust %>%
  ...
  subset(select = c(date, Industry, y)) %>%
  rename(RP = y) %>%
  inner_join(PRS_Betas_STL_s40_robust %>%
               filter(component == "Trend") %>%
               subset(select = c(date, Industry, Factors, y)) %>%
               rename(Betas = y) %>%
               pivot_wider(names_from = Factors, values_from = Betas) %>%
...
All_FF5_ECC <- ECC_AM_FF5 %>%
  ...
  rename(AM = ECC) %>%
  cbind(ECC_GM_FF5$ECC) %>%
  rename(GM = `ECC_GM_FF5$ECC`) %>%
  cbind(ECC_FM_FF5$ECC) %>%
  rename(FM = `ECC_FM_FF5$ECC`) %>%
  cbind(ECC_FM_STL_FF5$ECC) %>%
  rename(FM_STL_7_N = `ECC_FM_STL_FF5$ECC`) %>%
  cbind(ECC_FM_STL_7_R_FF5$ECC) %>%
  rename(FM_STL_7_R = `ECC_FM_STL_7_R_FF5$ECC`) %>%
  cbind(ECC_FM_STL_15_N_FF5$ECC) %>%
  rename(FM_STL_15_N = `ECC_FM_STL_15_N_FF5$ECC`) %>%
  cbind(ECC_FM_STL_15_R_FF5$ECC) %>%
  rename(FM_STL_15_R = `ECC_FM_STL_15_R_FF5$ECC`) %>%
  cbind(ECC_FM_STL_40_N_FF5$ECC) %>%
  rename(FM_STL_40_N = `ECC_FM_STL_40_N_FF5$ECC`) %>%
  cbind(ECC_FM_STL_40_R_FF5$ECC) %>%
  rename(FM_STL_40_R = `ECC_FM_STL_40_R_FF5$ECC`) %>%
  inner_join(Port_FF5 %>%
               subset(select = c(date, Industry, RP)))
...
All_PRS_ECC <- ECC_AM_PRS %>%
  subset(select = c(date, Industry, ECC)) %>%
  rename(AM = ECC) %>%
  cbind(ECC_GM_PRS$ECC) %>%
  rename(GM = `ECC_GM_PRS$ECC`) %>%
  cbind(ECC_FM_PRS$ECC) %>%
  rename(FM = `ECC_FM_PRS$ECC`) %>%
  cbind(ECC_FM_STL_PRS$ECC) %>%
  rename(FM_STL_7_N = `ECC_FM_STL_PRS$ECC`) %>%
  cbind(ECC_FM_STL_7_R_PRS$ECC) %>%
  rename(FM_STL_7_R = `ECC_FM_STL_7_R_PRS$ECC`) %>%
  cbind(ECC_FM_STL_15_N_PRS$ECC) %>%
  rename(FM_STL_15_N = `ECC_FM_STL_15_N_PRS$ECC`) %>%
  cbind(ECC_FM_STL_15_R_PRS$ECC) %>%
  rename(FM_STL_15_R = `ECC_FM_STL_15_R_PRS$ECC`) %>%
  cbind(ECC_FM_STL_40_N_PRS$ECC) %>%
  rename(FM_STL_40_N = `ECC_FM_STL_40_N_PRS$ECC`) %>%
  cbind(ECC_FM_STL_40_R_PRS$ECC) %>%
  rename(FM_STL_40_R = `ECC_FM_STL_40_R_PRS$ECC`) %>%
  inner_join(Port_PRS %>%
               subset(select = c(date, Industry, RP)))

(All_FF5_ECC[,3:11] - All_FF5_ECC[,12])^2 %>%
  colSums() %>%
  as.matrix() %>%
  t() %>%
  as_tibble() %>%
  mutate(Model = "FF5") %>%
  rbind((All_PRS_ECC[,3:11] - All_PRS_ECC[,12])^2 %>%
          colSums() %>%
          as.matrix() %>%
          t() %>%
          as_tibble() %>%
          mutate(Model = "PRS")) %>%
  subset(select = c(10,1:9)) %>%

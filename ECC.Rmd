...
FM_FF5_Error <- cbind(Rolling_Error_Second[, 1], Rolling_Error) %>%
  subset(select = -Lambda0)
FM_FF5_Resid <- Rolling_FF5_Resid
...
# R_sq inverse
INV_sig_Inter <- FM_FF5_Error/INV_sig_Intra/cbind(1/sig_e[,1],rep(sig_e[,2],5))

sig_Inter <- cbind(sig_e[,1], (INV_sig_Intra[,2:6]*rep(sig_e[,2],5)/FM_FF5_Error[,2:6])) %>%
  tibble() %>%
  mutate(date = sig_e$date)

# Decomposition
SE_Decomposition <- sig_e %>%
  rename(sig_e = Resid) %>%
  inner_join(sig_Inter %>%
               pivot_longer(-date, names_to = "Factor", values_to = "sig_Inter")) %>%
  inner_join(sig_Intra %>%
               pivot_longer(-date, names_to = "Factor", values_to = "sig_Intra")) %>%
  inner_join(FM_FF5_Error %>%
               pivot_longer(-date, names_to = "Factor", values_to = "Lambda_Error")) %>%
  mutate(sig_e = log(sig_e)/log(Lambda_Error),
         sig_Inter = log(sig_Inter)/log(Lambda_Error),
         sig_Intra = log(sig_Intra)/log(Lambda_Error)) %>%
  select(c(date,Factor,sig_e,sig_Intra,sig_Inter))
...
SE_Decomposition %>%
  ...
  rbind(SE_Decomposition %>%
          mutate(Factor = "Overall") %>%
          pivot_longer(-c(date, Factor), names_to = "Type", values_to = "Percent") %>%
          select(!date) %>%
          group_by(Factor, Type) %>%
          summarise_all(funs(mean, median, variance = var(.), skewness, kurtosis))) %>%
  kable(align = "c", caption = "Fama Macbeth lambda SE Decomposition Percent") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  # Add header above
...
SE_Decomposition %>%
  ...
  labs(title= "Fama Macbeth lambda SE Component Decomposition",
       x = "date") + 
  theme(plot.title = element_text(hjust=0.5),
        axis.text.x = element_text(angle = 30, hjust = 1))
...
SE_Decomposition %>%
  write_csv("Lambda_SE_Percent.csv")
```
...
All_FF5_ECC %>%
  ...
  # fix order
  mutate(Method = factor(Method, levels = c("FM","FM_STL"))) %>%
  ggplot() +
  geom_density(aes(ECC, linetype = Method), adjust=1.5) +
...
Port_FF5_STL_s7_norobust %>%
  ...
  summarise(Trend = max(0, 1-var(Noise)/var(Noise+Trend)),
            Seasonal = max(0, 1-var(Noise)/var(Noise+Seasonal))) %>%
  pivot_longer(cols = c(Trend,Seasonal), names_to = "Component", values_to = "Strength") %>%
  subset(select = c(Component, Strength)) %>%
  mutate(Component = factor(Component, levels = c("Trend","Seasonal"))) %>%
  ungroup() %>%
  group_by(Component) %>%
  ggplot() +
  geom_density(aes(Strength, linetype = Component), adjust=1.5) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(y = "Density") +
  theme(axis.text.x = element_text(angle = 30, hjust = 1),
        legend.position="bottom")

# Figure 4 Plot the rolling betas box plot for Market
Port_FF5_Betas %>%
  subset(select = c(date, Industry, `Mkt-RF`)) %>%
  # pivot longer to make factors a category
  pivot_longer(cols = -c(date, Industry),
...
# Figure 5 Plot the rolling betas box plot for other 4 factors
Port_FF5_Betas %>%
  subset(select = -c(Alpha,`Mkt-RF`)) %>%
  # pivot longer to make factors a category
  pivot_longer(cols = -c(date, Industry),
...
# Figure 6 density plot of FM_FF5 lambda
FM_FF5 %>%
  mutate(Method = "FM") %>%
  rbind(FM_STL_FF5 %>%
          mutate(Method = "FM_STL")) %>%
  ...
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  ggplot() +
  geom_density(aes(Lambda, linetype = Method), adjust=1.5) +
  scale_linetype_manual(values=c("dotdash", "solid")) +
  labs(y = "Density") +
  facet_wrap(~ Factors,
             scales = "free_y") +
...
# Figure 7 time-series plot of the FM_FF5 lambda
FM_FF5 %>%
  mutate(Method = "FM") %>%
  rbind(FM_STL_FF5 %>%
          mutate(Method = "FM_STL")) %>%
...
# Figure 8 Lambda SE Decomposition
SE_Decomposition %>%
  mutate(date = transDate(date),
         Factor = factor(Factor, levels = unique(Factor))) %>%
  rename(Residual = sig_e,
         `Intergroup Variation` = sig_Inter,
         `Intragroup Variation` = sig_Intra) %>%
...
# Figure 9 Plot the standard error in second-step regression
FM_FF5_Error %>%
...
# Figure 10 Create the stats for Estimated ECC mean and Std across Industries
ECC_FM_FF5 %>%
  rename(FM = ECC) %>%
  select(c(date, FM)) %>%
...
# Figure 11 Create the stats for Estimated ECC mean and Std across time for each industry
ECC_FM_FF5 %>%
  rename(FM = ECC) %>%

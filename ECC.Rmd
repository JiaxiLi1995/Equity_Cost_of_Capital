...
Aux_result <- cbind(Rolling_Second[, 1], Aux_result) %>%
  subset(select = -Pi0)
Aux_se <- cbind(Rolling_Second[, 1], Aux_se) %>%
  subset(select = -Pi0)
...
Aux_result_Noise <- cbind(Rolling_Second[, 1], Aux_result_Noise) %>%
  subset(select = -Pi0)
Aux_se_Noise <- cbind(Rolling_Second[, 1], Aux_se_Noise) %>%
  subset(select = -Pi0)
...
Aux_result_Trend <- cbind(Rolling_Second[, 1], Aux_result_Trend) %>%
  subset(select = -Pi0)
Aux_se_Trend <- cbind(Rolling_Second[, 1], Aux_se_Trend) %>%
  subset(select = -Pi0)
...
# Add in the dates
Long_result <- cbind(Rolling_Second[, 1], Long_result) %>%
  subset(select = -Pi0)
Long_se <- cbind(Rolling_Second[, 1], Long_se) %>%
  subset(select = -Pi0)
Long_t_stat <- cbind(Rolling_Second[, 1], Long_t_stat) %>%
  subset(select = -Pi0)
Long_p_value <- cbind(Rolling_Second[, 1], Long_p_value) %>%
  subset(select = -Pi0)
Long_F_test <- cbind(Rolling_Second[, 1], Long_F_test)
...
Now, let's compare the long-regression lambdas with the original second-path and ACL results:
...
```{r Long_vs_ACL, cache = TRUE, eval=FALSE}
...
Now, we would like to compare the standard errors of the original second pass regression vs. ACL second pass regression.

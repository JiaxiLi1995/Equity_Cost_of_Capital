...
However, the eta might actually contain 3 components: eps_t, s_i, and eps_it. 

$$\eta_{it} = \epsilon_t + s_i + \epsilon_{it}$$
...
$$r_{it} = \beta_{it}*\lambda_t+\epsilon_t + s_i + \epsilon_{it}$$

Again, we would have:

$$E[\epsilon_t + s_i + \epsilon_{it}|i] = 0$$

However, the cross-sectional regression error at each time t:

$$E[\epsilon_t + s_i + \epsilon_{it}|t] = \epsilon_t+s_i$$
...
It seems that the both the trend and seasonal components are weak. No robust is stronger than robust, and smaller s_window woudl yield stronger components.
...
# Calculate the correlations between filtered and unfiltered lamdbas
FM_FF5 %>%

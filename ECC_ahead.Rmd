...
Sim_return %>%
  ...
  pivot_wider(names_from = Industry, values_from = "Value") %>%
  arrange(Strength) %>%
  kable(align = "c", caption = "STL Decomposition Component Strength for Sin Lambda Simulated Portfolio Risk Premium") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  collapse_rows(columns = 1)
...
Sim_return2 %>%
  ...
  pivot_wider(names_from = Industry, values_from = "Value") %>%
  arrange(Strength) %>%
  kable(align = "c", caption = "STL Decomposition Component Strength for Random Fourier Lambda Simulated Portfolio Risk Premium") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  collapse_rows(columns = 1)

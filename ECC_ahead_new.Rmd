...
In the factor premium estimation, we would use the information from the previous month to estimate the lambdas of the current month.

```{r change_month_for_second_step, cache = TRUE}
Port_FF5_Betas_lead <- Port_FF5_Betas %>%
  group_by(Industry) %>%
  mutate(date = lead(date)) %>%
  na.omit() %>%
  ungroup()

Port_PRS_Betas_lead <- Port_PRS_Betas %>%
  group_by(Industry) %>%
  mutate(date = lead(date)) %>%
  na.omit() %>%
  ungroup()
...
Second_Path <- Port_FF5 %>%
  subset(select = c(date, Industry, RP)) %>%
  inner_join(Port_FF5_Betas_lead) %>%
  subset(select = -Alpha) %>%
...
Second_Path <- Port_PRS %>%
  subset(select = c(date, Industry, RP)) %>%
  inner_join(Port_PRS_Betas_lead) %>%
  subset(select = -Alpha) %>%
  # get rid of Other category before regression
  filter(Industry != "Other") %>%
  ungroup()
...
# SST inverse
INV_sig_Intra_FF5 <- Port_FF5_Betas_lead %>%
...
sig_Intra_FF5 <- Port_FF5_Betas_lead %>%
  # get rid of Other category
  filter(Industry != "Other") %>%
  select(!c(Alpha,Industry)) %>%
  group_by(date) %>%
...
# SST inverse
INV_sig_Intra_PRS <- Port_PRS_Betas_lead %>%
  # get rid of Other category
  filter(Industry != "Other") %>%
  select(!c(Alpha,Industry)) %>%
  group_by(date) %>%
...
sig_Intra_PRS <- Port_PRS_Betas_lead %>%
  # get rid of Other category
  filter(Industry != "Other") %>%
  select(!c(Alpha,Industry)) %>%
  group_by(date) %>%
...
```{r save_decomposed_data, eval=FALSE}
...
# STL method with s_window = 7, no robust weight
FF5_Betas_STL_s7_norobust <- Port_FF5_Betas_lead %>%
  subset(select = -Alpha) %>%
...
# STL method with s_window = 7, with robust weight
FF5_Betas_STL_s7_robust <- Port_FF5_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
...
# STL method with s_window = 15, no robust weight
FF5_Betas_STL_s15_norobust <- Port_FF5_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
...
# STL method with s_window = 15, with robust weight
FF5_Betas_STL_s15_robust <- Port_FF5_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
...
# STL method with s_window = 40, no robust weight
FF5_Betas_STL_s40_norobust <- Port_FF5_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
...
# STL method with s_window = 40, with robust weight
FF5_Betas_STL_s40_robust <- Port_FF5_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
...
# STL method with s_window = 7, no robust weight
PRS_Betas_STL_s7_norobust <- Port_PRS_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
  do(model = stl_tibble(., "date", "Betas")) %>%
...
# STL method with s_window = 7, with robust weight
PRS_Betas_STL_s7_robust <- Port_PRS_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
  do(model = stl_tibble(., "date", "Betas", robust = TRUE)) %>%
...
# STL method with s_window = 15, no robust weight
PRS_Betas_STL_s15_norobust <- Port_PRS_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
  do(model = stl_tibble(., "date", "Betas", s_window = 15)) %>%
...
# STL method with s_window = 15, with robust weight
PRS_Betas_STL_s15_robust <- Port_PRS_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
  do(model = stl_tibble(., "date", "Betas", s_window = 15, robust = TRUE)) %>%
...
# STL method with s_window = 40, no robust weight
PRS_Betas_STL_s40_norobust <- Port_PRS_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
  do(model = stl_tibble(., "date", "Betas", s_window = 40)) %>%
...
# STL method with s_window = 40, with robust weight
PRS_Betas_STL_s40_robust <- Port_PRS_Betas_lead %>%
  subset(select = -Alpha) %>%
  pivot_longer(cols = -c(date, Industry), names_to = "Factors", values_to = "Betas") %>%
  group_by(Industry, Factors) %>%
  do(model = stl_tibble(., "date", "Betas", s_window = 40, robust = TRUE)) %>%
...
sim_return3 <- Port_FF5_Betas_lead %>%
...
For Periodic (Seasonal) component. based on the F-test result, we only reject the null hypothesis that the coefficients are not jointly different from 0 about half the time. Individually, we have the p-value about 35% of the time to be less than 0.05.
...
We obtain different results for trend component: The market component significance happens less frequent, and the HML and CMA component significance happen more frequent. 
...
The proportion of significant lambda numbers is closer to the ACL regression (or the trend auxiliary regression).
...
$$T_{i,t} = \alpha + \beta F_{i,t}, + \gamma P_{i,t} + \delta N_{i,t} + \epsilon$$

```{r Long_Panel_reg, cache = TRUE}
# Group the data and do the second pass regression
Long_data %>%
  lm(RP ~ . - date - Industry, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0"))
```

It seems that overall, the periodic(seasonal) component and Noise component are not orthogonal to the Trend. This is a result due to our local smoothing method.
...
In this session, we will apply the filter again to a new set of test portfolios. Lu Zhang at Ohio State has provided a huge amount of these test portfolios, and breaks them into some categories. We selected some of these portfolios: Friction	(30), Intangible (60), Investment (81), Profitability (66), VVG (63).

### S4-7-1 Strength of each component

First, we would like to see whether a strong periodic component is common in all testing portfolios.
...
```{r Other_Port_Strength, cache = TRUE}
# Load all the other portfolios
Other_Port <- rbind(
  read_excel(here("data/Test_Ports/Frictions_test_portfolios_090322.xlsx")) %>%
    mutate(Type = "Friction") %>%
    pivot_longer(-c(dateym, Type), names_to = "Port", values_to = "Return"),
  read_excel(here("data/Test_Ports/Intangibles_test_portfolios_090322.xlsx")) %>%
    mutate(Type = "Intangible") %>%
    pivot_longer(-c(dateym, Type), names_to = "Port", values_to = "Return"),
  read_excel(here("data/Test_Ports/Investment_test_portfolios_090222.xlsx")) %>%
    mutate(Type = "Investment") %>%
    pivot_longer(-c(dateym, Type), names_to = "Port", values_to = "Return"),
  read_excel(here("data/Test_Ports/Profitability_test_portfolios_090322.xlsx")) %>%
    mutate(Type = "Profitability") %>%
    pivot_longer(-c(dateym, Type), names_to = "Port", values_to = "Return"),
  read_excel(here("data/Test_Ports/VVG_test_portfolios_090222.xlsx")) %>%
    mutate(Type = "VVG") %>%
    pivot_longer(-c(dateym, Type), names_to = "Port", values_to = "Return")) %>%
  na.omit() %>%
  rename(date = dateym)

# Join the Other portfolios and FF5
Other_Port_FF5 <- Other_Port %>%
  ...
  arrange(Port) %>%
  # Locate the time and asset when 5-year rolling beta is possible to estimate (59 entries before, the PERMNO is the same, the date is 4 year 11 month less)
  mutate(Selection = (Port == lag(Port,59)) & ((date == (lag(date,59)+411))|(date == lag(date,59)+499)),
         Selection = replace_na(Selection,FALSE))

# Calculate the full sample betas
# Create the IDs for Industry
Other_ID <- Other_Port_FF5 %>%
  group_by(Port) %>%
  summarise(n = n()) %>%
  mutate(Ind_ID = 1:length(unique(Other_Port_FF5$Port))) %>%
  subset(select = -n)

# STL method with s_window = 7, no robust weight
Other_Port_STL_s7_norobust <- Other_Port_FF5 %>%
  subset(select = c(date, Port, Type, RP)) %>%
  group_by(Port) %>%
  do(model = stl_tibble(., "date", "RP")) %>%
  .$model %>%
  do.call(what = rbind.data.frame)

# Strength of all portfolios
Other_Port_Component_Strength <- Other_Port_STL_s7_norobust %>%
  subset(select = -c(x, RP)) %>%
  pivot_wider(names_from = "component", values_from = "y") %>%
  group_by(Port, Type) %>%
  summarise(FT = max(0, 1-var(Noise)/var(Noise+Trend)),
            FS = max(0, 1-var(Noise)/var(Noise+Seasonal))) %>%
  subset(select = c(Type, Port, FT, FS)) %>%
  rename(`Strength of Trend` = FT,
         `Strength of Periodic` = FS)

# Create a table showing the percentage of portfolios have larger strenth of periodic
Other_Port_Component_Strength %>%
  mutate(`Strong Period` = `Strength of Trend` < `Strength of Periodic`) %>%
  group_by(Type) %>%
  summarise(Percentage = mean(`Strong Period`),
            Number = n()) %>%
  mutate(Percentage = scales::label_percent()(Percentage)) %>%
  kable(align = "c", caption = "Strength of Period > Strength of Trend") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T)

# Save as csv
# write_csv(Other_Port_Component_Strength, "Other_Port_Component_Strength.csv")

# Plot them
Other_Port_Component_Strength %>%
  ggplot() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", size = 0.75) +
  geom_point(aes(`Strength of Trend`, `Strength of Periodic`, color = Type)) +
  coord_cartesian(xlim = c(0.1, 0.45), ylim = c(0.1, 0.45)) +
  labs(title= "Strength of Components for other Test Portfolios") + 
  theme(plot.title = element_text(hjust=0.5))


```

As the table suggests, all of the test portfolios we have suffer from strong periodic component (stronger than trend strength). This shows that the ACL regression can be used in the general setting of ECC estimation.

### S4-7-2 5-year Rolling FF5 Betas

In this section, we estimate the FF5 betas applying a 5-year rolling window.


```{r Other_Rolling_FF5, cache = TRUE}
# Create the estimated portfolio beta matrix
Other_Port_FF5_Betas_lead <- Other_Port_FF5 %>%
  rename(Alpha = RP)

# apply the roll_regres to calculate the 5-year rolling beta (60 data points)
temp_out <- roll_regres(RP ~ . -Type -Port -date -Selection, Other_Port_FF5, width = 60L)
# Save as tibble
temp_betas <- temp_out$coefs %>% as_tibble()

# Assign them to Betas
for (j in 4:(length(Other_Port_FF5_Betas_lead)-1)) {
  Other_Port_FF5_Betas_lead[,j] <- temp_betas[,j-3]
}

# Filter the needed ones and remove Selection
Other_Port_FF5_Betas_lead <- Other_Port_FF5_Betas_lead %>%
  filter(Selection == TRUE) %>%
  select(-Selection) %>%
  group_by(Port) %>%
  mutate(date = lead(date)) %>%
...
In this session, we will perform the second-pass and the same set of Auxilary regressions as S4-6-1. Note that the Trend Auxilary regressoin is also the ACL regression. Orignial is referring to the Fama Macbeth second-pass regression.

```{r Other_Aux_reg, cache = TRUE}
## Create the portfolio for Other_FF5_reg
Other_FF5_data <- Other_Port_STL_s7_norobust %>%
  filter(component == "Original") %>%
  subset(select = c(date, Port, y)) %>%
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
  select(-Alpha) %>%
  select(-Type) %>%
  ungroup()

# Group the data and do the second pass regression
Other_FF5_reg <- Other_FF5_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coefficients())

# Obtain the se
Other_FF5_reg_se <- Other_FF5_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Std. Error"])
...
# Obtain the t_stat
Other_FF5_reg_t_stat <- Other_FF5_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "t value"])
...
# Obtain the p_value
Other_FF5_reg_p_value <- Other_FF5_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Pr(>|t|)"])
...
# perform F test p_value
Other_FF5_reg_F_test <- Other_FF5_data %>%
  group_by(date) %>%
  do(f_p_value = lm(RP ~ . - date - Port, data = .) %>%
       summary() %>%
       .$fstatistic %>%
       pf(q = .[1], df1 = .[2], df2 = .[3], lower.tail = FALSE) %>%
       .["value"])
...
# Unwrap the results
Other_FF5_result <- do.call(rbind.data.frame, Other_FF5_reg$model) %>%
  as_tibble()
Other_FF5_se <- do.call(rbind.data.frame, Other_FF5_reg_se$model) %>%
  as_tibble()
Other_FF5_t_stat <- do.call(rbind.data.frame, Other_FF5_reg_t_stat$model) %>%
  as_tibble()
Other_FF5_p_value <- do.call(rbind.data.frame, Other_FF5_reg_p_value$model) %>%
  as_tibble()
Other_FF5_F_test <- do.call(rbind.data.frame, Other_FF5_reg_F_test$f_p_value) %>%
  as_tibble()


# Rename the variables
names(Other_FF5_result) = names(FF5)[-length(FF5)]
names(Other_FF5_result)[1] = "Pi0"
names(Other_FF5_se) = names(FF5)[-length(FF5)]
names(Other_FF5_se)[1] = "Pi0"
names(Other_FF5_t_stat) = names(FF5)[-length(FF5)]
names(Other_FF5_t_stat)[1] = "Pi0"
names(Other_FF5_p_value) = names(FF5)[-length(FF5)]
names(Other_FF5_p_value)[1] = "Pi0"
names(Other_FF5_F_test) = "f_p_value"

# Add in the dates
Other_FF5_result <- cbind(Other_FF5_reg[, 1], Other_FF5_result) %>%
  subset(select = -Pi0)
Other_FF5_se <- cbind(Other_FF5_reg[, 1], Other_FF5_se) %>%
  subset(select = -Pi0)
Other_FF5_t_stat <- cbind(Other_FF5_reg[, 1], Other_FF5_t_stat) %>%
  subset(select = -Pi0)
Other_FF5_p_value <- cbind(Other_FF5_reg[, 1], Other_FF5_p_value) %>%
  subset(select = -Pi0)
Other_FF5_F_test <- cbind(Other_FF5_reg[, 1], Other_FF5_F_test)


# Plot the Other_FF5_result
Other_FF5_result %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "FF5 Lambdas for second pass regression",
       subtitle = "Based on Other Portfolios",
       x = "date", y = "Pi") +
  theme(plot.title = element_text(hjust=0.5),
        plot.subtitle = element_text(hjust=0.5))


# Plot the Other_FF5_t_stat
Other_FF5_t_stat %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi_t_stat") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "t Statistics for FF5 Lambdas for second pass regression",
       subtitle = "Based on Other Portfolios",
       x = "date", y = "Density") +
  theme(plot.title = element_text(hjust=0.5),
        plot.subtitle = element_text(hjust=0.5))
...
"Proportion of siginificant F statistics"
mean(Other_FF5_F_test$f_p_value < 0.05)
...
## Create the portfolio for Other_Aux_reg
Other_Aux_data <- Other_Port_STL_s7_norobust %>%
  filter(component == "Seasonal") %>%
  subset(select = c(date, Port, y)) %>%
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
...
# Group the data and do the second pass regression
Other_Aux_reg <- Other_Aux_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coefficients())

# Obtain the se
Other_Aux_reg_se <- Other_Aux_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Std. Error"])
...
# Obtain the t_stat
Other_Aux_reg_t_stat <- Other_Aux_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "t value"])
...
# Obtain the p_value
Other_Aux_reg_p_value <- Other_Aux_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Pr(>|t|)"])
...
# perform F test p_value
Other_Aux_reg_F_test <- Other_Aux_data %>%
  group_by(date) %>%
  do(f_p_value = lm(RP ~ . - date - Port, data = .) %>%
       summary() %>%
       .$fstatistic %>%
...
# Unwrap the results
Other_Aux_result <- do.call(rbind.data.frame, Other_Aux_reg$model) %>%
  as_tibble()
Other_Aux_se <- do.call(rbind.data.frame, Other_Aux_reg_se$model) %>%
  as_tibble()
Other_Aux_t_stat <- do.call(rbind.data.frame, Other_Aux_reg_t_stat$model) %>%
  as_tibble()
Other_Aux_p_value <- do.call(rbind.data.frame, Other_Aux_reg_p_value$model) %>%
  as_tibble()
Other_Aux_F_test <- do.call(rbind.data.frame, Other_Aux_reg_F_test$f_p_value) %>%
  as_tibble()


# Rename the variables
names(Other_Aux_result) = names(FF5)[-length(FF5)]
names(Other_Aux_result)[1] = "Pi0"
names(Other_Aux_se) = names(FF5)[-length(FF5)]
names(Other_Aux_se)[1] = "Pi0"
names(Other_Aux_t_stat) = names(FF5)[-length(FF5)]
names(Other_Aux_t_stat)[1] = "Pi0"
names(Other_Aux_p_value) = names(FF5)[-length(FF5)]
names(Other_Aux_p_value)[1] = "Pi0"
names(Other_Aux_F_test) = "f_p_value"

# Add in the dates
Other_Aux_result <- cbind(Other_Aux_reg[, 1], Other_Aux_result) %>%
  subset(select = -Pi0)
Other_Aux_se <- cbind(Other_Aux_reg[, 1], Other_Aux_se) %>%
  subset(select = -Pi0)
Other_Aux_t_stat <- cbind(Other_Aux_reg[, 1], Other_Aux_t_stat) %>%
  subset(select = -Pi0)
Other_Aux_p_value <- cbind(Other_Aux_reg[, 1], Other_Aux_p_value) %>%
  subset(select = -Pi0)
Other_Aux_F_test <- cbind(Other_Aux_reg[, 1], Other_Aux_F_test)

# Plot the Other_Aux_result
Other_Aux_result %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
...
"Proportion of siginificant F statistics"
mean(Other_Aux_F_test$f_p_value < 0.05)
...
## Create the portfolio for Other_Aux_reg_Noise
Other_Aux_data_Noise <- Other_Port_STL_s7_norobust %>%
  filter(component == "Noise") %>%
  subset(select = c(date, Port, y)) %>%
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
...
# Group the data and do the second pass regression
Other_Aux_reg_Noise <- Other_Aux_data_Noise %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coefficients())

# Obtain the se
Other_Aux_reg_se_Noise <- Other_Aux_data_Noise %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Std. Error"])
...
# Obtain the t_stat
Other_Aux_reg_t_stat_Noise <- Other_Aux_data_Noise %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "t value"])
...
# Obtain the p_value
Other_Aux_reg_p_value_Noise <- Other_Aux_data_Noise %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Pr(>|t|)"])
...
# perform F test p_value
Other_Aux_reg_F_test_Noise <- Other_Aux_data_Noise %>%
  group_by(date) %>%
  do(f_p_value = lm(RP ~ . - date - Port, data = .) %>%
       summary() %>%
       .$fstatistic %>%
...
# Unwrap the results
Other_Aux_result_Noise <- do.call(rbind.data.frame, Other_Aux_reg_Noise$model) %>%
  as_tibble()
Other_Aux_se_Noise <- do.call(rbind.data.frame, Other_Aux_reg_se_Noise$model) %>%
  as_tibble()
Other_Aux_t_stat_Noise <- do.call(rbind.data.frame, Other_Aux_reg_t_stat_Noise$model) %>%
  as_tibble()
Other_Aux_p_value_Noise <- do.call(rbind.data.frame, Other_Aux_reg_p_value_Noise$model) %>%
  as_tibble()
Other_Aux_F_test_Noise <- do.call(rbind.data.frame, Other_Aux_reg_F_test_Noise$f_p_value) %>%
  as_tibble()


# Rename the variables
names(Other_Aux_result_Noise) = names(FF5)[-length(FF5)]
names(Other_Aux_result_Noise)[1] = "Pi0"
names(Other_Aux_se_Noise) = names(FF5)[-length(FF5)]
names(Other_Aux_se_Noise)[1] = "Pi0"
names(Other_Aux_t_stat_Noise) = names(FF5)[-length(FF5)]
names(Other_Aux_t_stat_Noise)[1] = "Pi0"
names(Other_Aux_p_value_Noise) = names(FF5)[-length(FF5)]
names(Other_Aux_p_value_Noise)[1] = "Pi0"
names(Other_Aux_F_test_Noise) = "f_p_value"

# Add in the dates
Other_Aux_result_Noise <- cbind(Other_Aux_reg_Noise[, 1], Other_Aux_result_Noise) %>%
  subset(select = -Pi0)
Other_Aux_se_Noise <- cbind(Other_Aux_reg_Noise[, 1], Other_Aux_se_Noise) %>%
  subset(select = -Pi0)
Other_Aux_t_stat_Noise <- cbind(Other_Aux_reg_Noise[, 1], Other_Aux_t_stat_Noise) %>%
  subset(select = -Pi0)
Other_Aux_p_value_Noise <- cbind(Other_Aux_reg_Noise[, 1], Other_Aux_p_value_Noise) %>%
  subset(select = -Pi0)
Other_Aux_F_test_Noise <- cbind(Other_Aux_reg_Noise[, 1], Other_Aux_F_test_Noise)

# Plot the Other_Aux_result
Other_Aux_result_Noise %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  mutate(date = transDate(date)) %>%
...
"Proportion of siginificant F statistics"
mean(Other_Aux_F_test_Noise$f_p_value < 0.05)

"Proportion of siginificant t statistics"
((Other_Aux_p_value_Noise %>%
    select(-date))<0.05) %>%
  colMeans()

## Create the portfolio for Other_Aux_reg_Trend
Other_Aux_data_Trend <- Other_Port_STL_s7_norobust %>%
  filter(component == "Trend") %>%
  subset(select = c(date, Port, y)) %>%
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
...
# Group the data and do the second pass regression
Other_Aux_reg_Trend <- Other_Aux_data_Trend %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coefficients())

# Obtain the se
Other_Aux_reg_se_Trend <- Other_Aux_data_Trend %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Std. Error"])
...
# Obtain the t_stat
Other_Aux_reg_t_stat_Trend <- Other_Aux_data_Trend %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "t value"])
...
# Obtain the p_value
Other_Aux_reg_p_value_Trend <- Other_Aux_data_Trend %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Pr(>|t|)"])
...
# perform F test p_value
Other_Aux_reg_F_test_Trend <- Other_Aux_data_Trend %>%
  group_by(date) %>%
  do(f_p_value = lm(RP ~ . - date - Port, data = .) %>%
       summary() %>%
       .$fstatistic %>%
...
# Unwrap the results
Other_Aux_result_Trend <- do.call(rbind.data.frame, Other_Aux_reg_Trend$model) %>%
  as_tibble()
Other_Aux_se_Trend <- do.call(rbind.data.frame, Other_Aux_reg_se_Trend$model) %>%
  as_tibble()
Other_Aux_t_stat_Trend <- do.call(rbind.data.frame, Other_Aux_reg_t_stat_Trend$model) %>%
  as_tibble()
Other_Aux_p_value_Trend <- do.call(rbind.data.frame, Other_Aux_reg_p_value_Trend$model) %>%
  as_tibble()
Other_Aux_F_test_Trend <- do.call(rbind.data.frame, Other_Aux_reg_F_test_Trend$f_p_value) %>%
  as_tibble()


# Rename the variables
names(Other_Aux_result_Trend) = names(FF5)[-length(FF5)]
names(Other_Aux_result_Trend)[1] = "Pi0"
names(Other_Aux_se_Trend) = names(FF5)[-length(FF5)]
names(Other_Aux_se_Trend)[1] = "Pi0"
names(Other_Aux_t_stat_Trend) = names(FF5)[-length(FF5)]
names(Other_Aux_t_stat_Trend)[1] = "Pi0"
names(Other_Aux_p_value_Trend) = names(FF5)[-length(FF5)]
names(Other_Aux_p_value_Trend)[1] = "Pi0"
names(Other_Aux_F_test_Trend) = "f_p_value"

# Add in the dates
Other_Aux_result_Trend <- cbind(Other_Aux_reg_Trend[, 1], Other_Aux_result_Trend) %>%
  subset(select = -Pi0)
Other_Aux_se_Trend <- cbind(Other_Aux_reg_Trend[, 1], Other_Aux_se_Trend) %>%
  subset(select = -Pi0)
Other_Aux_t_stat_Trend <- cbind(Other_Aux_reg_Trend[, 1], Other_Aux_t_stat_Trend) %>%
  subset(select = -Pi0)
Other_Aux_p_value_Trend <- cbind(Other_Aux_reg_Trend[, 1], Other_Aux_p_value_Trend) %>%
  subset(select = -Pi0)
Other_Aux_F_test_Trend <- cbind(Other_Aux_reg_Trend[, 1], Other_Aux_F_test_Trend)

# Plot the Other_Aux_result
Other_Aux_result_Trend %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  mutate(date = transDate(date)) %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Coefficients from Auxiliary Regression for Trend (ACL)",
       subtitle = "Based on Other Portfolios",
       x = "date", y = "Pi") +
  theme(plot.title = element_text(hjust=0.5),
        plot.subtitle = element_text(hjust=0.5))
...
Other_Aux_t_stat_Trend %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "t Statistics from Auxiliary Regression for Trend (ACL)",
       subtitle = "Based on Other Portfolios",
       x = "date", y = "Density") +
  theme(plot.title = element_text(hjust=0.5),
        plot.subtitle = element_text(hjust=0.5))
...
"Proportion of siginificant F statistics"
mean(Other_Aux_F_test_Trend$f_p_value < 0.05)

"Proportion of siginificant t statistics"
((Other_Aux_p_value_Trend %>%
    select(-date))<0.05) %>%
  colMeans()
...
rbind(
  ...
  pivot_longer(-c(stat, Y_type, Factors), names_to = "Stat", values_to = "Values") %>%
  pivot_wider(names_from = "stat", values_from = "Values") %>%
  kable(align = "c", caption = "Auxilary Regression Results for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1:2, bold = T) %>%
  collapse_rows(columns = 1:2)
...
rbind(
  ...
  summarise(Sig_Percent = mean(Values < 0.05)) %>%
  pivot_wider(values_from = Sig_Percent, names_from = Factors) %>%
  kable(align = "c", caption = "Auxilary Regression Coefficient Significance Percentage for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  collapse_rows(columns = 1)
...
```{r Other_Panel_Check, cache = TRUE}
## Create the second-path portfolio for estimation lambda
Second_Path_component_Other <- Other_Port_STL_s7_norobust %>%
  subset(select = c(date, Port, y, component)) %>%
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
  subset(select = -Type) %>%
  subset(select = -Alpha) %>%
...
# do the regression using panel data
Second_component_panel_Other <- Second_Path_component_Other %>%
  select(-c(Port, component)) %>%
  group_by(component_num) %>%
  do(model = lm(RP ~ . - date - component_num, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")))

"Auxilary Panel Regressoin for Other Portfolios"
# Original Panel
"Original"
Second_component_panel_Other$model[[1]]

# Trend Panel
"Trend"
Second_component_panel_Other$model[[2]]

# Periodic (Seasonal) Panel
"Periodic (Seasonal)"
Second_component_panel_Other$model[[3]]

# Noise Panel
"Noise"
Second_component_panel_Other$model[[4]]

```

We get similar results as before, only that at each t, all factors are significant in the auxilary regressions more. The Panel regression still suggests that the trend should be explained by the factors while we do not have enough evidence to support that the other components (Noise, Periodic) are explained by the factors. This still validates our Approach of ACL regression.
...
```{r Other_Long_reg, cache = TRUE}
## Create the portfolio for Other_Long_reg
Other_Long_data <- Other_Port_STL_s7_norobust %>%
  filter(component %in% c("Trend", "Seasonal", "Noise")) %>%
  pivot_wider(names_from = "component", values_from = "y") %>%
  subset(select = c(date, Port, Trend, Seasonal, Noise)) %>%
  rename(RP = Trend) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
  select(-Alpha) %>%
  select(-Type) %>%
...
# Group the data and do the second pass regression
Other_Long_reg <- Other_Long_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coefficients())

# Obtain the se
Other_Long_reg_se <- Other_Long_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Std. Error"])
...
# Obtain the t_stat
Other_Long_reg_t_stat <- Other_Long_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "t value"])
...
# Obtain the p_value
Other_Long_reg_p_value <- Other_Long_data %>%
  group_by(date) %>%
  do(model = lm(RP ~ . - date - Port, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Pr(>|t|)"])
...
# perform F test p_value
Other_Long_reg_F_test <- Other_Long_data %>%
  group_by(date) %>%
  do(f_p_value = lm(RP ~ . - date - Port, data = .) %>%
       summary() %>%
       .$fstatistic %>%
...
# Unwrap the results
Other_Long_result <- do.call(rbind.data.frame, Other_Long_reg$model) %>%
  as_tibble()
Other_Long_se <- do.call(rbind.data.frame, Other_Long_reg_se$model) %>%
  as_tibble()
Other_Long_t_stat <- do.call(rbind.data.frame, Other_Long_reg_t_stat$model) %>%
  as_tibble()
Other_Long_p_value <- do.call(rbind.data.frame, Other_Long_reg_p_value$model) %>%
  as_tibble()
Other_Long_F_test <- do.call(rbind.data.frame, Other_Long_reg_F_test$f_p_value) %>%
  as_tibble()


# Rename the variables
names(Other_Long_result) = c("Pi0", "Periodic", names(FF5)[-length(FF5)])
names(Other_Long_result)[3] = "Noise"
names(Other_Long_se) = c("Pi0", "Periodic", names(FF5)[-length(FF5)])
names(Other_Long_se)[3] = "Noise"
names(Other_Long_t_stat) = c("Pi0", "Periodic", names(FF5)[-length(FF5)])
names(Other_Long_t_stat)[3] = "Noise"
names(Other_Long_p_value) = c("Pi0", "Periodic", names(FF5)[-length(FF5)])
names(Other_Long_p_value)[3] = "Noise"
names(Other_Long_F_test) = "f_p_value"

# Add in the dates
Other_Long_result <- cbind(Other_Long_reg[, 1], Other_Long_result) %>%
  subset(select = -Pi0)
Other_Long_se <- cbind(Other_Long_reg[, 1], Other_Long_se) %>%
  subset(select = -Pi0)
Other_Long_t_stat <- cbind(Other_Long_reg[, 1], Other_Long_t_stat) %>%
  subset(select = -Pi0)
Other_Long_p_value <- cbind(Other_Long_reg[, 1], Other_Long_p_value) %>%
  subset(select = -Pi0)
Other_Long_F_test <- cbind(Other_Long_reg[, 1], Other_Long_F_test)

# Plot the Other_Long_result
Other_Long_result %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Coefficients from Other Portfolio Long Regression for Trend",
       x = "date", y = "Pi") +
  theme(plot.title = element_text(hjust=0.5))


# Plot the Other_Long_t_stat
Other_Long_t_stat %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Pi_t_stat") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "t Statistics from Other Portfolio Long Regression for Trend",
       x = "date", y = "Density") +
  theme(plot.title = element_text(hjust=0.5))

"Proportion of siginificant F statistics for Other Portfolio Long Regression"
mean(Other_Long_F_test$f_p_value < 0.05)

"Proportion of siginificant t statistics for Other Portfolio Long Regression"
((Other_Long_p_value %>%
    select(-date))<0.05) %>%
  colMeans()
```

Similarly, the proportion of significant lambda numbers is closer to the ACL regression (or the trend auxiliary regression).

Now, let's compare the long-regression lambdas with the original second-path result:
  
```{r Other_Long_vs_SP, cache = TRUE}
# This is a table for Other_Long regression coefficient result comparing with Original Second pass regression
Other_Long_vs_SP <- inner_join(
  inner_join(Other_Long_result %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Long_Estimate"),
             Other_Long_se %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Long_Std. Error")) %>%
    subset(!(Factors %in% c("Periodic", "Noise"))),
  inner_join(Other_FF5_result %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Original_Estimate"),
             FM_FF5_Error %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Original_Std. Error"))
) %>%
  mutate(Diff_Estimate = Long_Estimate - Original_Estimate,
         `Diff_Std. Error` = `Long_Std. Error` - `Original_Std. Error`,
         Diff_t_stat = Diff_Estimate/`Diff_Std. Error`,
         Diff_P_value = pt(Diff_t_stat, df = 2*(nrow(Other_Long_result) - 1), lower.tail = FALSE))
...
Other_Long_vs_SP %>%
  ...
  pivot_longer(-c(Factors, Type), names_to = "Stat", values_to = "Values") %>%
  pivot_wider(names_from = "Type", values_from = "Values") %>%
  kable(align = "c", caption = "Long vs. Original Regression Results for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1:2, bold = T) %>%
  collapse_rows(columns = 1:2)

# This is a table for Other_Long regression coefficient result comparing with Original Second pass regression percentage of significantly different coefficients
Other_Long_vs_SP %>%
  mutate(Factors = factor(Factors, levels = unique(.$Factors))) %>%
  select(c(Factors,Diff_P_value)) %>%
  group_by(Factors) %>%
  summarise(Sig_Percent = mean(Diff_P_value < 0.05)) %>%
  kable(align = "c", caption = "Long vs. Original Difference Significance Percentage for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T)

```

It seems that 20\% of the time, the result from long regression would be different from the second-step regression. Similar to the result in s4-6-3, this result is consistent with previous short regressions, which showed Period and Noise are associated with $\hat{\beta}$ and would introduce a bias to the second-step regression. With more portfolios, it seems though the results are less frequently different.


```{r Other_Long_vs_ACL, cache = TRUE}
# This is a table for Other_Long regression coefficient result comparing with ACL regression
Other_Long_vs_ACL <- inner_join(
  inner_join(Other_Long_result %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Long_Estimate"),
             Other_Long_se %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Long_Std. Error")) %>%
    subset(!(Factors %in% c("Periodic", "Noise"))),
  inner_join(Other_Aux_result_Trend %>%
               pivot_longer(-date, names_to = "Factors", values_to = "ACL_Estimate"),
             Aux_se_Trend %>%
               pivot_longer(-date, names_to = "Factors", values_to = "ACL_Std. Error"))
) %>%
...
Other_Long_vs_ACL %>%
  ...
  pivot_longer(-c(Factors, Type), names_to = "Stat", values_to = "Values") %>%
  pivot_wider(names_from = "Type", values_from = "Values") %>%
  kable(align = "c", caption = "Long vs. ACL Regression Results for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1:2, bold = T) %>%
  collapse_rows(columns = 1:2)
...
Other_Long_vs_ACL %>%
  ...
  group_by(Factors) %>%
  summarise(Sig_Percent = mean(Diff_P_value < 0.05)) %>%
  kable(align = "c", caption = "Long vs. ACL Difference Significance Percentage for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T)

```

It seems that less than 10\% of the time, the result from Long regression would be different from the ACL regression. This result is consistent with previous short regressions, which showed Period and Noise are associated with $\hat{\beta}$ and would introduce a bias to the second-step regression. This result further shows the validity of ACL approach.

Finally, let's compare the ACL-regression lambdas with the original second-path result:

```{r Other_Original_vs_ACL, cache = TRUE}
# This is a table for Original FM regression coefficient result comparing with ACL regression
Other_Original_vs_ACL <- inner_join(
  inner_join(Other_FF5_result %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Original_Estimate"),
             Other_FF5_se %>%
               pivot_longer(-date, names_to = "Factors", values_to = "Original_Std. Error")),
...
Other_Original_vs_ACL %>%
  ...
  pivot_longer(-c(Factors, Type), names_to = "Stat", values_to = "Values") %>%
  pivot_wider(names_from = "Type", values_from = "Values") %>%
  kable(align = "c", caption = "Original FM vs. ACL Regression Results for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1:2, bold = T) %>%
  collapse_rows(columns = 1:2)
...
Original_vs_ACL %>%
  ...
  group_by(Factors) %>%
  summarise(Sig_Percent = mean(Diff_P_value < 0.05)) %>%
  kable(align = "c", caption = "Original FM vs. ACL Difference Significance Percentage for Other Portfolios") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T)
...
```{r Other_Long_Panel_reg, cache = TRUE}
# Group the data and do the second pass regression
Other_Long_data %>%
  lm(RP ~ . - date - Port, data = .) %>%
  coeftest(., vcov = vcovHC(., type = "HC0"))
```
...
Similar to s-4-6-4, we would like to compare the original second pass regression vs. ACL second pass regression in a rolling panel setting:
...
```{r Other_Second_Pass_Rolling_Panel_Comparison, cache = TRUE, eval=FALSE}
# Fama Macbeth method
# Create the second-path portfolio for estimation lambda: 1 stands for original regression, 2 stands for ACL regression
Other_Original_long <- Other_FF5_data %>%
  subset(select = c(date, Port, RP)) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
  subset(select = -Alpha) %>%
  subset(select = -Type) %>%
  ungroup() %>%
  pivot_longer(-c(date, Port), values_to = "Value", names_to = "Factors") %>%
  mutate(Reg_type = 1)

Other_ACL_long <- Other_Port_STL_s7_norobust %>%
  filter(component == "Trend") %>%
  subset(select = c(date, Port, y)) %>%
  rename(RP = y) %>%
  inner_join(Other_Port_FF5_Betas_lead) %>%
...
Other_Rolling_Panel_Long <- rbind(Other_Original_long, Other_ACL_long)

# apply the tk_augment_lags to calculate the 1,3,5-year rolling lambda (12,36,60 data points)
Other_Second_Path_lag_1 <- Other_Rolling_Panel_Long %>%
  group_by(Port, Factors, Reg_type) %>%
  tk_augment_lags(Value, .lags = 1:(1*12-1)) %>%
  na.omit() %>%
  pivot_longer(-c(date, Port, Factors, Reg_type), values_to = "Values", names_to = "Lags") %>%
  mutate(Values = as.numeric(Values)) %>%
  pivot_wider(names_from = "Factors", values_from = "Values") %>%
  select(-Lags) %>%
  mutate(Panel = 1)

Other_Second_Path_lag_3 <- Other_Rolling_Panel_Long %>%
  group_by(Port, Factors, Reg_type) %>%
  tk_augment_lags(Value, .lags = 1:(3*12-1)) %>%
  na.omit() %>%
  pivot_longer(-c(date, Port, Factors, Reg_type), values_to = "Values", names_to = "Lags") %>%
...
Other_Second_Path_lag_5 <- Other_Rolling_Panel_Long %>%
  group_by(Port, Factors, Reg_type) %>%
  tk_augment_lags(Value, .lags = 1:(5*12-1)) %>%
  na.omit() %>%
  pivot_longer(-c(date, Port, Factors, Reg_type), values_to = "Values", names_to = "Lags") %>%
...
Other_Second_Path_lag <- rbind((Other_Rolling_Panel_Long %>% pivot_wider(names_from = "Factors", values_from = "Value") %>% mutate(Panel = 0)),Other_Second_Path_lag_1,Other_Second_Path_lag_3,Other_Second_Path_lag_5) %>%
  ungroup() %>%
  select(-Port)

# Group the data and do the second pass regression
Other_Rolling_Second <- Other_Second_Path_lag %>%
  group_by(date, Reg_type, Panel) %>%
  do(model = lm(RP ~ . - date - Reg_type - Panel, data = .) %>%
       coefficients())

# try the standard error
Other_Rolling_Error_Second <- Other_Second_Path_lag %>%
  group_by(date, Reg_type, Panel) %>%
  do(model = lm(RP ~ . - date - Reg_type - Panel, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Std. Error"])
...
# try the t stat
Other_Rolling_t_stat_Second <- Other_Second_Path_lag %>%
  group_by(date, Reg_type, Panel) %>%
  do(model = lm(RP ~ . - date - Reg_type - Panel, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "t value"])
...
# try the p_value
Other_Rolling_p_value_Second <- Other_Second_Path_lag %>%
  group_by(date, Reg_type, Panel) %>%
  do(model = lm(RP ~ . - date - Reg_type - Panel, data = .) %>%
       coeftest(., vcov = vcovHC(., type = "HC0")) %>%
       .[, "Pr(>|t|)"])
...
# Unwrap the results
Other_Rolling_Lambdas <- do.call(rbind.data.frame, Other_Rolling_Second$model) %>%
  as_tibble()
Other_Rolling_Error <- do.call(rbind.data.frame, Other_Rolling_Error_Second$model) %>%
  as_tibble()
Other_Rolling_t_stat <- do.call(rbind.data.frame, Other_Rolling_t_stat_Second$model) %>%
  as_tibble()
Other_Rolling_p_value <- do.call(rbind.data.frame, Other_Rolling_p_value_Second$model) %>%
  as_tibble()

# Rename the variables
names(Other_Rolling_Lambdas) = names(FF5)[-length(FF5)]
names(Other_Rolling_Lambdas)[1] = "Lambda0"
names(Other_Rolling_Error) = names(FF5)[-length(FF5)]
names(Other_Rolling_Error)[1] = "Lambda0"
names(Other_Rolling_t_stat) = names(FF5)[-length(FF5)]
names(Other_Rolling_t_stat)[1] = "Lambda0"
names(Other_Rolling_p_value) = names(FF5)[-length(FF5)]
names(Other_Rolling_p_value)[1] = "Lambda0"

# Add in the dates and other info
Other_Panel_Long <- cbind(Other_Rolling_Second[, c("date", "Reg_type", "Panel")], Other_Rolling_Lambdas) %>%
  subset(select = -Lambda0) %>%
  mutate(Reg_type = factor(Reg_type, 1:2, c("Original", "ACL")))
Other_Panel_Long_Lambda0 <- cbind(Other_Rolling_Second[, c("date", "Reg_type", "Panel")], Other_Rolling_Lambdas) %>%
  mutate(Reg_type = factor(Reg_type, 1:2, c("Original", "ACL"))) %>%
  subset(select = c(date,Reg_type, Panel, Lambda0))
Other_Panel_Long_Error <- cbind(Other_Rolling_Second[, c("date", "Reg_type", "Panel")], Other_Rolling_Error) %>%
  subset(select = -Lambda0) %>%
  mutate(Reg_type = factor(Reg_type, 1:2, c("Original", "ACL")))
Other_Panel_Long_t_stat <- cbind(Other_Rolling_Second[, c("date", "Reg_type", "Panel")], Other_Rolling_t_stat) %>%
  subset(select = -Lambda0) %>%
  mutate(Reg_type = factor(Reg_type, 1:2, c("Original", "ACL")))
Other_Panel_Long_p_value <- cbind(Other_Rolling_Second[, c("date", "Reg_type", "Panel")], Other_Rolling_p_value) %>%
  subset(select = -Lambda0) %>%
  mutate(Reg_type = factor(Reg_type, 1:2, c("Original", "ACL")))
```

```{r Other_Second_Pass_Rolling_Panel_Comparison_Plotting, cache = TRUE, eval=FALSE}
# Plot the Other_Panel_Long
Other_Panel_Long %>%
  subset(Panel == 0) %>%
  pivot_longer(cols = -c(date, Reg_type, Panel), names_to = "Factors", values_to = "RP") %>%
  # Fix the order of Factors
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Estimated Factors Premium at each t for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Estimated Factors Premium from Rolling 1-year Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Estimated Factors Premium from Rolling 3-year Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Estimated Factors Premium from Rolling 5-year Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Estimated Factors Premium from ACL Rolling Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Estimated Factors Premium from Original FM Rolling Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))


# Plot the Other_Panel_Long_Error
Other_Panel_Long_Error %>%
  subset(Panel == 0) %>%
  pivot_longer(cols = -c(date, Reg_type, Panel), names_to = "Factors", values_to = "RP") %>%
  # Fix the order of Factors
  mutate(Factors = factor(Factors, level= unique(.$Factors))) %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Factors Premium Standard Error at each t for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long_Error %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Factors Premium Standard Error from Rolling 1-year Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long_Error %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Factors Premium Standard Error from Rolling 3-year Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long_Error %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Factors Premium Standard Error from Rolling 5-year Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long_Error %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Factors Premium Standard Error from ACL Rolling Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
...
Other_Panel_Long_Error %>%
  ...
  facet_wrap(~ Factors,
             scales = "free_y") +
  labs(title= "Factors Premium Standard Error from Original FM Rolling Panel for Other Portfolios",
       x = "date", y = "Lambda") +
  theme(plot.title = element_text(hjust=0.5),
        legend.position = c(0.8, 0.2))
```

From the figures above, we get similar results. It seems that with longer panel:

1. the estimates of ACL and Fama Macbeth regression are converging

2. the standard error of ACL and Fama Macbeth regression are converging

For any given length of Panel, ACL panel regression can produce lambda with relatively low risk, even for the new portfolios.
...
```{r ACL_Paper_Figures, cache=TRUE}
...
# Figure 14 HML factor premium estimates from different horizon regression
Panel_Long %>%
  mutate(date = transDate(date)) %>%
  mutate(Horizon = factor(as.character(Panel), labels = c("One-month Return Horizon", "One-year Return Horizon", "Three-year Return Horizon", "Five-year Return Horizon"))) %>%
  mutate(Method = factor(Reg_type, labels = c("FM", "FM-Filtered"))) %>%
  ggplot(aes(date, HML, linetype = Method)) +
  geom_line() +
  scale_x_date(minor_breaks = "1 year") +
  scale_linetype_manual(values=c("dotted", "solid")) +
  facet_wrap(~ Horizon,
             scales = "free_y") +
  labs(x = "Date", y = "Lambda") +
  theme(legend.position="bottom")


# Figure 15 HML factor premium standard errors from different horizon regression
Panel_Long_Error %>%
  mutate(date = transDate(date)) %>%
  mutate(Horizon = factor(as.character(Panel), labels = c("One-month Return Horizon", "One-year Return Horizon", "Three-year Return Horizon", "Five-year Return Horizon"))) %>%
  mutate(Method = factor(Reg_type, labels = c("FM", "FM-Filtered"))) %>%
...
# Figure 16 Create the stats for Estimated ECC mean and Std across Industries
ECC_FM_FF5 %>%
  # Remove the Other Industry
  filter(Industry != "Other") %>%
  rename(FM = ECC) %>%
...
# Figure 17 Create the stats for Estimated ECC mean and Std across time for each industry
ECC_FM_FF5 %>%
  # Remove the Other Industry
  filter(Industry != "Other") %>%
  rename(FM = ECC) %>%
...
```{r ACL_Paper_Appendix_Figures, cache=TRUE}
############################################################
# Simulation Appendix
...
############################################################
# PRS Appendix
# Figure A1 Plot the rolling betas box plot for Market
Port_PRS_Betas %>%
  subset(select = c(date, Industry, `Mkt-RF`)) %>%
  # pivot longer to make factors a category
  pivot_longer(cols = -c(date, Industry),
...
# Figure A2 Plot the rolling betas box plot for other 4 factors
Port_PRS_Betas %>%
  subset(select = -c(Alpha,`Mkt-RF`)) %>%
  # pivot longer to make factors a category
  pivot_longer(cols = -c(date, Industry),
...
# Figure A3 density plot of FM_PRS lambda
FM_PRS %>%
  mutate(Method = "FM") %>%
  rbind(FM_STL_PRS %>%
          mutate(Method = "FM-Filtered")) %>%
...
# Figure A4 time-series plot of the FM_PRS lambda
FM_PRS %>%
  mutate(Method = "FM") %>%
  rbind(FM_STL_PRS %>%
          mutate(Method = "FM-Filtered")) %>%
...
# Figure A5 Lambda SE Decomposition
SE_Decomposition_PRS %>%
  mutate(date = transDate(date),
         Factor = factor(Factor, levels = unique(Factor))) %>%
...
# Figure A6 Plot the standard error in second-step regression
FM_PRS_Error %>%
  pivot_longer(cols = -date, names_to = "Factors", values_to = "Standard Error") %>%
  mutate(Method = "FM") %>%
  rbind(FM_STL_PRS_Error %>%
...
# Figure A7 Create the stats for Estimated ECC mean and Std across Industries
ECC_FM_PRS %>%
  # Remove the Other Industry
  filter(Industry != "Other") %>%
  rename(FM = ECC) %>%
...
# Figure A8 Create the stats for Estimated ECC mean and Std across time for each industry
ECC_FM_PRS %>%
  # Remove the Other Industry
  filter(Industry != "Other") %>%
  rename(FM = ECC) %>%
...
```{r ACL_Paper_Tables, cache=TRUE}
# Create the numbers for construction industry
All_FF5_ECC %>%
  filter(Industry == "Cnstr") %>%
  subset(select = c(date, FM)) %>%
  summarise(Mean = round(mean(FM), digits = 5),
            Mean_Annual = round(mean(FM)*1200, digits = 2),
            StdDev = round(sd(FM), digits = 5),
            StdDev_Annual = round(sd(FM)*sqrt(12)*100, digits = 1),
            `25th Pct` = round(quantile(FM, .25), digits = 3),
            `25th Pct Annual` = round(quantile(FM, .25)*1200, digits = 1),
            `75th Pct` = round(quantile(FM, .75), digits = 3),
            `75th Pct Annual` = round(quantile(FM, .75)*1200, digits = 1)) %>%
  kable(align = "c", caption = "Construction Industry ECC Summary") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  collapse_rows(columns = 1)
...
# Table1 Summary of Trend and Periodic Component Tests for other portfolios
Other_Port_Component_Strength %>%
  ungroup() %>%
  mutate(`Portfolio Group` = replace(Type, Type == "VVG", "Value vs. Growth")) %>%
  rbind(Other_Port_Component_Strength %>%
  ungroup() %>%
  mutate(`Portfolio Group` = "Total")) %>%
  # fix the Portfolio Group order
  mutate(`Portfolio Group` = factor(`Portfolio Group`, levels = c("Friction", "Intangible", "Investment", "Profitability", "Value vs. Growth", "Total"))) %>%
  pivot_longer(-c(Type, Port, `Portfolio Group`), names_to = "Strength of Group", values_to = "Value") %>%
  # fix the Strength of Group order
  mutate(`Strength of Group` = factor(`Strength of Group`, levels = unique(.$`Strength of Group`))) %>%
  select(-c(Port, Type)) %>%
  group_by(`Strength of Group`, `Portfolio Group`) %>%
  summarise(Mean = round(mean(Value), digits = 3),
            Median = round(median(Value), digits = 3),
            StdDev = round(sd(Value), digits = 3),
            `25th Pct` = round(quantile(Value, .25), digits = 3),
            `75th Pct` = round(quantile(Value, .75), digits = 3),
            N = n()) %>%
  kable(align = "c", caption = "Other Portfolio Strength") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  collapse_rows(columns = 1)


# Table2 FF5 lambda Summary
FM_FF5 %>%
  mutate(method = "FM") %>%
  rbind(FM_STL_FF5 %>%
...
# Table3 Auxiliary Regression Results
Second_component_panel$model[[2]][2:6, 1:2] %>%
  cbind(Second_component_panel$model[[3]][2:6, 1:2]) %>%
  cbind(Second_component_panel$model[[4]][2:6, 1:2]) %>%
  round(., digits = 5) %>%
  kable(align = "c", caption = "Auxiliary Regression Results") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T)


# Table4 ACL vs. ACL Aug. Risk Premium Estimates
FM_STL_FF5 %>%
  mutate(method = "ACL") %>%
  rbind(Long_result %>%
          select(-c(Periodic, Noise)) %>%
          mutate(method = "ACL Aug.")) %>%
  # fix the method order
  mutate(method = factor(method, levels = unique(.$method))) %>%
  select(-date) %>%
  pivot_longer(-method, names_to = "Factors", values_to = "Value") %>%
  ...
  summarise(Mean = round(mean(Value), digits = 4),
            Median = round(median(Value), digits = 4),
            StdDev = round(sd(Value), digits = 4),
            `Skew.` = round(skewness(Value), digits = 4),
            `Kurt` = round(kurtosis(Value), digits = 4),
            `25th Pct` = round(quantile(Value, .25), digits = 4),
            `75th Pct` = round(quantile(Value, .75), digits = 4)) %>%
  kable(align = "c", caption = "Comparison of Risk Premium Estimates") %>%
  kable_styling(font_size = 10, "striped", full_width = F) %>%
  column_spec(1, bold = T) %>%
  collapse_rows(columns = 1)
```

```{r ACL_Paper_Appendix_Tables, cache=TRUE}
# Table A1 PRS lambda Summary
FM_PRS %>%
  mutate(method = "FM") %>%
  rbind(FM_STL_PRS %>%
          mutate(method = "Fil. FM")) %>%
